import { Component, OnInit, ViewEncapsulation, AfterViewInit, Pipe, ChangeDetectorRef, PipeTransform, ViewChildren, QueryList } from '@angular/core';
...
export class TestComponent implements OnInit {
  ...
  openClosedData(value) {
    ...
    if (value == 'Program Management') {
      ...
      this.shortnamewithSKU = this.shortName + "_" + this.SKU;
      let payload = {
        "Type": 1
      }
      this.apiService.getFilters(payload).subscribe((res: any) => {
      ...
      if (this.program == "Program") {
        ...
        if (this.Intents == 'COE Management/ Lead') {
          ...
          if (this.domain == undefined) {
            this._snackBar.open("Please select Group Domain", "Done", {
              duration: 2000,
            });
          } else {
            this.dataView = true;
            this.GetTrendChartByDomain(this.domain);
  ...
  sceCountCOE;
  implementedCOE;
  monthlyAverageCounts;
  backlogPercentage;
  averageCountinfomationCOE;
  ageingIndicatorOpen;
  ageingIndicatorClosed;
  coeLeadcount(status) {
    let obj = {
    ...
    this.apiService.getCOELeadcount(obj).subscribe((res: any) => {
      ...
      this.dcrCountCOE = res.dcrCount[0].dcrCount;
      this.sceCountCOE = res.sceCount[0].sceCount;
      this.implementedCOE = res.implemented[0].verifiedCount;
      this.monthlyAverageCounts = res.monthlyAverageCounts[0].counts;
      this.backlogPercentage = res.backlogPercentage[0].percentage;
      this.averageCountinfomationCOE = res.averageCountinfomation[0].count;
      this.ageingIndicatorOpen = res.ageingIndicator[0].counts;
      this.ageingIndicatorClosed = res.ageingIndicator[1].counts;

    })
  }

  trendChartByDomainInfo;
  GetTrendChartByDomain(domain) {
    let obj = {
      "platform": this.shortName + "_" + this.SKU,
      "ChartType": "week",
      "domain": domain
  ...
  groupDomainList;
  groupDomainData() {
    let payload = { "platform": this.shortName + "_" + this.SKU };
    this.apiService.GetGroupDomain(payload).subscribe((res: any) => {

      this.groupDomainList = res.domainlistGroupInfo;
      TREE_DATA = [];
  ...
  nodeSelectionToggle(node: GameNode): void {
    ...
    this.domain = this.selectedGroupDomain;
    this.groupDomainPlaceholderTxt = this.selectedGroupDomain;
    // this.preSelectedList = this.selectedGroupDomain;
  ...
  sampleData(filter, chartType, type) {
    ...
    let t = this.router.serializeUrl(this.router.createUrlTree(['sample'], { queryParams: { CHARTNO: filter, FILTER: chartType, type: type, platform: this.program } }))
  ...
  report() {
    let t = this.router.serializeUrl(this.router.createUrlTree(['mail'], { queryParams: { platform: this.program } }))
    window.open("#" + t, '_blank')
  }
