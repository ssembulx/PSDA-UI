...
export class HeaderComponent implements OnInit {
  ...
  setInterFunc() {
    let setInter = setInterval(() => {
      if (this.selectedPlatform != undefined || this.selectedPlatform != null) {
        ...
        this.apiService
          .refreshStatusInformation(paylod)
          .subscribe((res: any) => {
    ...
    this.aRouter.queryParamMap.subscribe((data: any) => {
      if (JSON.stringify(data.params) != '{}') {
        ...
        this.apiService.getLastDataTime().subscribe((res: any) => {
          ...
          this.timelog = this.timeQuery.find(
            (d) => d.platformName == this.platformName
          ).timeLog;
  ...
  report() {
    this.router.navigate(['report'], {
      queryParams: { platform: this.selectedPlatform, operation: 'EditQuery' },
    });
  }
  tableDetails(filter) {
    let t = this.router.serializeUrl(
      this.router.createUrlTree(['component_mapped'])
    );
    window.open('#' + t, '_blank');
  }
  tableDetails1(filter) {
    let t = this.router.serializeUrl(
      this.router.createUrlTree(['component_unmapped'])
    );
    window.open('#' + t, '_blank');
  }
  platformEdit(platform: any) {
    this.router.navigate(['query'], {
      queryParams: { platform: platform, operation: 'EditQuery' },
    });
  }

  platformRemove(platform: any) {
    this.router.navigate(['query'], {
      queryParams: { platform: platform, operation: 'RemoveQuery' },
    });
  }
  user: boolean = false;
