import { Component, OnInit, AfterViewInit, ViewChild, Pipe, PipeTransform, ChangeDetectorRef, ViewChildren, QueryList } from '@angular/core';
...
import { identifierModuleUrl, isNgTemplate, ThisReceiver } from '@angular/compiler';
...
export interface PeriodicElement {
  selection_column: string, id: string, promoted_ID: string, title: string, status: string, exposure: string, domain: string, domain_debug: string, target: string,
  progress: string, sysdebug_forum: string, reason: string, isolated: string, priority: string, component: string, owner: string, next_step: string, days_open: string,
  board: string, regression: string, from_subject: string, submitted_by: string, ext_access: string, submitter_org: string, soc_stepping: string, target_impact: string,
  impact: string, submitted_date: string, closed_date: string, closed_reason: string, repro_on_rvp: string, open_date: string, sysdebug: string, implemented_date: string, verified_date: string,
  days_triage: string, qrc_vector: string, days_decimal_triage: string, days_validating: string, days_development: string, days_decimal_debugging: string, tag: string, count: string,
  updated_by: string, updated_date: string
}
...
export class SampleComponent implements OnInit, AfterViewInit {
  ...
  getActiveCountResultInfo: any;
  getClosedCountResultInfo: any;
  getTotalCountResultInfo: any;
  openClosedHSDESResult: any;
  fileName = 'ExcelSheet';
  ...
  getDomainAndExposureHSDESDetails: any;
  getDefectsVsRegressionHSDESDetails: any;
  getIngredientsExposureHSDESResult: any;
  ...
  selected_countvalue = 0;
  public columnShowHideList: any = [];
  editedRows: any[];
  OriginalData: any;
  getCriticalQRCvectorHSDESData: any[];
  downloadSelection: PeriodicElement[];
  details = [];
  columnFilter = {
    "status": "",
    "exposure": "",
  ...
  filter: any;
  platform: any;
  constructor(private apiService: ServiceAPIService, private aRouter: ActivatedRoute, private dialogRef: MatDialog, private changeDetectorRefs: ChangeDetectorRef, private notifier: NotifierService) {
    ...
    this.aRouter.queryParamMap.subscribe((data: any) => {
      ...
      if (JSON.stringify(data.params) != '{}') {
        ...
        let payload =
        {
          ...
          "CHARTNO": data.params.CHARTNO,
          "FILTER": data.params.FILTER,
          "isOpenClosed": false
        }

        if (data.params.type == "active" && !data.params.isOpenClosed) {

          this.apiService.getActiveCountResult(payload).subscribe((res: any) => {
            this.getActiveCountResultInfo = res.getActiveCountResultInfo;
            this.dataSource.data = this.getActiveCountResultInfo
            this.OriginalData = JSON.parse(JSON.stringify(this.getActiveCountResultInfo));
            this.masterToggle()
          })
        } else if (data.params.type == "cslp_data" && !data.params.isOpenClosed) {
          let payload =
          {
            "platform": data.params.platform,
            "customer_detail": data.params.FILTER,
            "status": data.params.status,
            "exposure": data.params.CHARTNO,
            "value": data.params.value
          }

          this.apiService.GetSightingAndExposure(payload).subscribe((res: any) => {
            this.getClosedCountResultInfo = res.allHSDESInfo;
            console.log(this.getClosedCountResultInfo);
            this.dataSource.data = this.getClosedCountResultInfo;
            this.OriginalData = JSON.parse(JSON.stringify(this.getClosedCountResultInfo));
            this.masterToggle()
          })
        } else if (data.params.type == "verified" && !data.params.isOpenClosed) {
          let payload =
          {
            "platform": data.params.platform,
          }
          this.apiService.GetStatusVerifiedHSDESResult(payload).subscribe((res: any) => {
            this.getClosedCountResultInfo = res.statusVerifiedHSDESDetails;
            console.log(this.getClosedCountResultInfo);
            this.dataSource.data = this.getClosedCountResultInfo;
            this.OriginalData = JSON.parse(JSON.stringify(this.getClosedCountResultInfo));
            this.masterToggle()
          })
        } else if (data.params.type == "closed" && !data.params.isOpenClosed) {
          let payload =
          {
            "platform": data.params.platform,
            "CHARTNO": data.params.CHARTNO,
          ...
          this.apiService.getClosedCountResult(payload).subscribe((res: any) => {
            this.getClosedCountResultInfo = res.getClosedCountResultInfo;
            console.log(this.getClosedCountResultInfo);
            this.dataSource.data = this.getClosedCountResultInfo;
            this.OriginalData = JSON.parse(JSON.stringify(this.getClosedCountResultInfo));
            this.masterToggle()
          })
        } else if (data.params.type == "Total" && !data.params.isOpenClosed) {
          let payload =
          {
            "platform": data.params.platform,
            "CHARTNO": data.params.CHARTNO,
          ...
          this.apiService.getTotalCountResult(payload).subscribe((res: any) => {
            this.getTotalCountResultInfo = res.getTotalCountResultInfo;
            console.log(this.getTotalCountResultInfo);
            this.dataSource.data = this.getTotalCountResultInfo
            this.OriginalData = JSON.parse(JSON.stringify(this.getTotalCountResultInfo));
            this.masterToggle()
          })
        } else if (data.params.isOpenClosed == 'true' && !data.params.trend_chart) {

          let payload = {
            "platform": data.params.platform,
            "CountType": data.params.COUNTTYPE,
            "ChartDate": data.params.CHARTDATE,
            "ChartType": data.params.CHARTTYPE,
            "Exposure": data.params.exposure
          }

          this.apiService.getOpenCloseHSDESResult(payload).subscribe((res: any) => {
            this.openClosedHSDESResult = res.getOpenCloseHSDESInfo;
            console.log(this.openClosedHSDESResult);
            this.dataSource.data = this.openClosedHSDESResult;
            this.OriginalData = JSON.parse(JSON.stringify(this.openClosedHSDESResult));
            this.masterToggle()

          })
        } else if (data.params.CHARTTYPE == "domainAndExposureHSDEC" && !data.params.isOpenClosed) {
          let payload =
          {
            "platform": data.params.platform,
            "DomainName": data.params.DomainName,
            "Exposure": data.params.Exposure
          }
          this.apiService.getDomainAndExposureHSDESResult(payload).subscribe((res: any) => {
            this.getDomainAndExposureHSDESDetails = res.getDomainAndExposureHSDESDetails;
            console.log(this.getDomainAndExposureHSDESDetails);
            this.dataSource.data = this.getDomainAndExposureHSDESDetails;
            this.OriginalData = JSON.parse(JSON.stringify(this.getDomainAndExposureHSDESDetails));
            this.masterToggle()
          })
        } else if (data.params.CHARTTYPE == "defectRegressionHSDEC" && !data.params.isOpenClosed) {
          let payload =
          {
            "Platform": data.params.platform,
            "Ingredient": data.params.Ingredient,
            "ChartType": data.params.ChartType,
            "Exposure": data.params.Exposure
          }
          this.apiService.getDefectsVsRegressionHSDESResult(payload).subscribe((res: any) => {
            this.getDefectsVsRegressionHSDESDetails = res.getDefectsVsRegressionHSDESDetails;
            console.log(this.getDefectsVsRegressionHSDESDetails);
            this.dataSource.data = this.getDefectsVsRegressionHSDESDetails;
            this.OriginalData = JSON.parse(JSON.stringify(this.getDefectsVsRegressionHSDESDetails));
            this.masterToggle()
          })
        } else if (data.params.CHARTTYPE == "ingredientExposureHSDEC" && !data.params.isOpenClosed) {
          let payload =
          {
            "platform": data.params.platform,
            "Ingredient": data.params.Ingredient,
          ...
          this.apiService.getIngredientExposureHSDESResult(payload).subscribe((res: any) => {
            this.getIngredientsExposureHSDESResult = res.getIngredientsAndExposureHSDESDetails;
            console.log(this.getIngredientsExposureHSDESResult);
            this.dataSource.data = this.getIngredientsExposureHSDESResult;
            this.OriginalData = JSON.parse(JSON.stringify(this.getIngredientsExposureHSDESResult));
            this.masterToggle()
          })
        } else if (data.params.type == "GetCriticalQRCvectorHSDESResult" && !data.params.isOpenClosed) {
          let payload =
          {
            "QRCVector": 'WSSDebug',
            "DefectFactor": '',
          ...
          this.apiService.getCriticalQRCvectorHSDESResult(payload).subscribe((res: any) => {
            this.getCriticalQRCvectorHSDESData = res.getCriticalQRCvectorHSDESDetails;
            console.log(this.getCriticalQRCvectorHSDESData);
            this.dataSource.data = this.getCriticalQRCvectorHSDESData;
            this.OriginalData = JSON.parse(JSON.stringify(this.getCriticalQRCvectorHSDESData));
            this.masterToggle()
          })
        } else if (data.params.type == "edited" && !data.params.isOpenClosed) {
          let payload =
          {
            "platform": data.params.platform
          }
          this.apiService.GetUpdateDataHSDES(payload).subscribe((res: any) => {
            this.getCriticalQRCvectorHSDESData = res.getAllUpdateHsdesDetails;
            console.log(this.getCriticalQRCvectorHSDESData);
            this.dataSource.data = this.getCriticalQRCvectorHSDESData;
            this.OriginalData = JSON.parse(JSON.stringify(this.getCriticalQRCvectorHSDESData));
            this.masterToggle()
        ...
        else if (data.params.isOpenClosed == 'true' && data.params.trend_chart == 'true') {
          let value = data.params.Domain
          if (value == 'Group Domain') {
            value = 'all'
          }
          let payload =
          {
          ...
          this.apiService.GetCalculationsTrendChartByDomain(payload).subscribe((res: any) => {
            this.getCriticalQRCvectorHSDESData = res.getOpenCloseHSDESInfo;
            console.log(this.getCriticalQRCvectorHSDESData);
            this.dataSource.data = this.getCriticalQRCvectorHSDESData;
            this.OriginalData = JSON.parse(JSON.stringify(this.getCriticalQRCvectorHSDESData));
            this.masterToggle()
        ...
        this.apiService.getTotalCountResult(payloadClosed).subscribe((res: any) => {
          this.getTotalCountResultInfo = res.getTotalCountResultInfo;
          console.log(this.getTotalCountResultInfo);
          this.dataSource.data = this.getTotalCountResultInfo;
          this.OriginalData = JSON.parse(JSON.stringify(this.getTotalCountResultInfo));
  ...
  isAllSelected() {
    ...
    this.selected_countvalue = this.downloadSelection.length;
