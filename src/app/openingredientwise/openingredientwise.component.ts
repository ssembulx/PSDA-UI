import { Component, OnInit, AfterViewInit, Pipe, PipeTransform, ViewChildren, QueryList } from '@angular/core';
...
@Component({
  selector: 'app-openingredientwise',
  templateUrl: './openingredientwise.component.html',
  styleUrls: ['./openingredientwise.component.css']
})
export class OpeningredientwiseComponent implements OnInit, AfterViewInit {
  ...
  OriginalData: any;
  getIngredientWiseCounts = []; getIngredientWiseHSDESInfo: any;
  platform: any;
  getIngredientData: { platform: any; ingredient: any; };
  columnFilter = {
    "status": "",
    "exposure": "",
    "domain": "",
  ...
  ngOnInit(): void {
    ...
    this.aRouter.queryParamMap.subscribe((data: any) => {
      ...
      if (JSON.stringify(data.params) != '{}') {
        if (data.params.tabletype == "IngredientWise") {
          ...
          this.apiService.getIngredientWiseResult(this.getIngredientData).subscribe((res: any) => {
            ...
            res.getIngredientWiseCounts.forEach(d => {
              d['disabled'] = this.selectedTarget.includes(d.ingredientcategory) ? true : false;
              d['checked'] = this.selectedTarget.includes(d.ingredientcategory) ? true : false;
            });

            this.getIngredientWiseCounts = res.getIngredientWiseCounts;
            this.getIngredientWiseHSDESInfo = res.getIngredientWiseHSDESInfo;

            console.log(this.getIngredientWiseHSDESInfo);
            this.dataSource.data = this.getIngredientWiseHSDESInfo;
            this.OriginalData = this.getIngredientWiseHSDESInfo
            this.masterToggle()
          })
        }
      } else {
  ...
  master_change(e: any): void {
    for (let value of Object.values(this.getIngredientWiseCounts)) {
      value.checked = this.master_checked;
    }
    console.log(this.getIngredientWiseCounts)
    this.callapi();
  }
  list_change() {
    let checked_count = 0;
    ...
    if (checked_count > 0 && checked_count < this.getIngredientWiseCounts.length) {
      // If some checkboxes are checked but not all; then set Indeterminate state of the master to true.
      this.master_indeterminate = true;
    } else if (checked_count == this.getIngredientWiseCounts.length) {
  ...
  callapi() {
    let array = this.getIngredientWiseCounts
    let currenttarget = []
    for (let step = 0; step < array.length; step++) {
      if (array[step].checked == true) {
        currenttarget.push(array[step].ingredientcategory)
      }
    }
  ...
  initialLoad = true;
  apiCallTargetData(setTarget) {
    this.getIngredientData = {
      "platform": this.platform,
      "ingredient": setTarget
    }
    this.apiService.getIngredientWiseResult(this.getIngredientData).subscribe((res: any) => {

      // res.getIngredientWiseCounts.forEach(d => {
      //   d['valueSet'] = this.selectedTarget.includes(d.ingredientcategory) ? true : false;
      // });
  ...
  //selectall for table
  // selectTargetData(event,ingredient){
  //   this.selection.clear();
  //   if(event.checked){   
  //     if(!this.selectedTarget.includes(ingredient)){ 
  //       this.selectedTarget.push(ingredient)
  //     }
  //   }else{
  //     let index = this.selectedTarget.indexOf(ingredient)
  //     if(index > -1){
  //       this.selectedTarget.splice(index,1)
  //     }
  //   }
  //   this.apiCallTargetData(this.selectedTarget.join(","));
