...
@Component({
  selector: 'app-opendomainwise',
  templateUrl: './opendomainwise.component.html',
  styleUrls: ['./opendomainwise.component.css']
})
export class OpendomainwiseComponent implements OnInit, AfterViewInit {
  ...
  OriginalData: any;
  getDomainDebugDate: { domain: string; type: string; };
  getDomainWiseCounts = [];
  getDomainWiseHSDESInfo: any;
  selectedTarget = [];
  platform: any;
  getDomainWiseDate: { platform: any; domain: any; type: string; };
  columnFilter = {
    "status": "",
    "exposure": "",
    "domain": "",
  ...
  ngOnInit(): void {
    ...
    this.aRouter.queryParamMap.subscribe((data: any) => {
      ...
      if (JSON.stringify(data.params) != '{}') {
        if (data.params.tabletype == "DomainWise" || data.params.tabletype == "DomainDebug") {
          ...
          this.apiService.getDomainWiseResult(this.getDomainWiseDate).subscribe((res: any) => {
            ...
            this.dataSource.data = this.getDomainWiseHSDESInfo;
  ...
  //   this.apiService.getDomainWiseResult(this.getDomainWiseDate).subscribe((res: any) => {
  //     res.getDomainWiseCounts.forEach(d => {
  //       d['valueSet'] = this.selectedTarget.includes(d.domain) ? true : false;
  //     });
  //     this.getDomainWiseCounts = res.getDomainWiseCounts;
  //     this.getDomainWiseHSDESInfo = res.getDomainWiseHSDESInfo;
