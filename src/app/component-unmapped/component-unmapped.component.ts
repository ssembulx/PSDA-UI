...
@Component({
  selector: 'app-component-unmapped',
  templateUrl: './component-unmapped.component.html',
  styleUrls: ['./component-unmapped.component.css']
})
export class ComponentUnmappedComponent implements OnInit {
  idVale: number;
  @ViewChild('addMainComponent') addMainComponent: TemplateRef<any>;
  @ViewChild('addSubComponent') addSubComponent: TemplateRef<any>;
  ...
  editedComponent: any;
  deleteComponent: any;
  newMainComponentName: string = null;
  changedDomainvalue: any;

  constructor(private helper: HelperService, private notifier: NotifierService, private apiService: ServiceAPIService, private aRouter: ActivatedRoute, private dialogRef: MatDialog) {
  ...
  ngOnInit(): void {
    this.helper.GetUser().subscribe(user => {
      ...
      if (user != null) {
        ...
        this.apiService.GetunmappedComponent(payload1).subscribe((res: any) => {
          this.getOpenStatusOnlyResultInfo = res.getMappedComDeleteStatus;
    ...
    if (this.selection.selected.length > 0) {
      ...
      this.selection.selected.forEach(element => {
        ...
        finalList.forEach(ele => {
          if(ele == "mainComponentName"){
            subArray.push("UnMapped");
          }else{
  ...
  // unmapped component
  openAddDialog() {
    const dialogRef = this.dialogRef.open(this.addMainComponent, {
      height: 'auto',
      width: '450px',
      disableClose: true
    });
  ...
  openAddSubDialog() {
    const dialogRef = this.dialogRef.open(this.addSubComponent, {
      height: 'auto',
      width: '450px',
      disableClose: true
    });
  ...
  getValueMethod(event) {
    this.newMainComponentName = event.target.value;
  }
  newSubComponentName;
  getSubValueMethod(event) {
    this.newSubComponentName = event.target.value;
  }
  onSubYesClick() {
    if (this.newSubComponentName != undefined || this.newSubComponentName != null) {
      let username = document.getElementById('username').innerHTML;
      let payload = {
        "ComponentName": this.newSubComponentName,
        "Type": 5,
        "User": username,
      }

      this.apiService.AddNewSubComponent(payload).subscribe((res: any) => {
        console.log("testdata")
        if (res) {
          this.dialogRef.closeAll();
          this.showNotification('success', 'Sub Component has been added successfully');
        }
      })
      this.refreshTable();
      this.ngOnInit();
  ...
  onYesClick() {
    if (this.newMainComponentName != undefined || this.newMainComponentName != null) {
      let username = document.getElementById('username').innerHTML;
      let payload = {
        "ComponentName": this.newMainComponentName,
        "Type": 4,
        "ComponentID": "",
        "ISDeleted": "",
        "User": username,
      ...
      this.apiService.AddNewComponent(payload).subscribe((res: any) => {
        console.log("testdata")
        if (res) {
          this.dialogRef.closeAll();
          this.showNotification('success', 'Component has been added successfully');
        }
      })
      this.refreshTable();
      this.ngOnInit();
  ...
  getChangeddomainvalue(value) {
    this.changedDomainvalue = value;
  ...
  saveUnMappedComponent(mappedComponentID, mainComponentName, componentName, createdby, createdDate) {
    ...
    let payload2 = {
      ...
      "ISDeleted": "",
      "User": username,
      "MappedComponentID": mappedComponentID,
      "MainComponentName": id.componentName
    }
    this.apiService.EditSaveUnMappedComponent(payload2).subscribe((res: any) => {
      console.log("testdata" + res.getmaincomponent)
      if (res) {
        this.dialogRef.closeAll();
        this.showNotification('success', 'Component has been saved successfully');
        this.changeComponentDropdown(value);
        this.selected = value;
      }
    })
