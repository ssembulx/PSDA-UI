...
@Component({
  selector: 'app-qrc-hsdes',
  templateUrl: './qrc-hsdes.component.html',
  styleUrls: ['./qrc-hsdes.component.css']
})
export class QrcHSDESComponent implements OnInit, AfterViewInit {
  ...
  ngOnInit(): void {
    ...
    this.aRouter.queryParamMap.subscribe((data: any) => {
      ...
      if (JSON.stringify(data.params) != '{}') {
        if (data.params.type == "GetCriticalQRCvectorHSDESResult") {
    ...
    this.apiService.getCriticalQRCvector(this.payload).subscribe((res: any) => {
  ...
  apiCall() {
    this.apiService.getCriticalQRCvector(this.payload).subscribe((res: any) => {
      this.getCriticalQRCvectorHSDESData = res.getQRCVectorHSDESInfo;
      console.log(this.getCriticalQRCvectorHSDESData);
      this.dataSource.data = this.getCriticalQRCvectorHSDESData;
      this.OriginalData = this.getCriticalQRCvectorHSDESData
      this.masterToggle()
    })
  }
