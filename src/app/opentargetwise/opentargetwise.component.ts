...
@Component({
  selector: 'app-opentargetwise',
  templateUrl: './opentargetwise.component.html',
  styleUrls: ['./opentargetwise.component.css']
})
export class OpentargetwiseComponent implements OnInit, AfterViewInit {
  ...
  id: string;
  getTargetWiseCounts = [];
  target: any; target_count: any;
  getTargetWiseHSDESInfo: any;
  ...
  select_all = false;
  selectAlltarget = false;
  allSelectedTarget: any = [];
  ...
  selectedTarget: any = [];
  platform: string;
  getTargetWiseData: { platform: string; target: any; };
  columnFilter = {
    "status": "",
    "exposure": "",
    "domain": "",
  ...
  ngOnInit(): void {
    ...
    this.aRouter.queryParamMap.subscribe((data: any) => {
      ...
      if (JSON.stringify(data.params) != '{}') {
        if (data.params.tabletype == "TargetWise") {
          ...
          this.apiService.getTargetWiseResult(this.getTargetWiseData).subscribe((res: any) => {
            res.getTargetWiseCounts.forEach(d => {
              //d['valueSet'] = this.selectedTarget.includes(d.target) ? true : false;
              d['disabled'] = this.selectedTarget.includes(d.target) ? true : false;
              d['checked'] = this.selectedTarget.includes(d.target) ? true : false;
            });
            this.getTargetWiseCounts = res.getTargetWiseCounts;

            console.log(this.getTargetWiseCounts)
            this.getTargetWiseHSDESInfo = res.getTargetWiseHSDESInfo;

            console.log(this.getTargetWiseHSDESInfo);
            this.dataSource.data = this.getTargetWiseHSDESInfo;
            this.OriginalData = this.getTargetWiseHSDESInfo
            this.masterToggle()
          })
        }
      } else {
  ...
  master_change(e: any): void {
    for (let value of Object.values(this.getTargetWiseCounts)) {
  ...
  list_change() {
    ...
    if (checked_count > 0 && checked_count < this.getTargetWiseCounts.length) {
      // If some checkboxes are checked but not all; then set Indeterminate state of the master to true.
      this.master_indeterminate = true;
    } else if (checked_count == this.getTargetWiseCounts.length) {
  ...
  callapi() {
    let array = this.getTargetWiseCounts
    let currenttarget = []
    for (let step = 0; step < array.length; step++) {
      if (array[step].checked == true) {
        currenttarget.push(array[step].target)
      }
    }
